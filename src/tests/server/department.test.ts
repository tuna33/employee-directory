...
describe("Department", () => {
  ...
  it("should get existing departments", async () => {
    ...
    await fetch("/api/departments/", { method: "GET" }).then((res) => {
  ...
  it("should create a valid department", async () => {
    await fetch("/api/departments/", {
      method: "POST",
      body: JSON.stringify(departmentInfoInstance),
    }).then((res) => {
  ...
  it("should delete a valid department", async () => {
    ...
    await fetch("/api/departments/1", { method: "DELETE" }).then((res) => {
  ...
  it("should update a valid department's info to valid info", async () => {
    const department = server.create("department", {
      info: departmentInfoInstance,
    });
    expect(department.id).toEqual("1");
    await fetch("/api/departments/1", {
      method: "PUT",
      body: JSON.stringify({ name: "Updated Department" }),
    }).then((res) => {
      expect(res.status).toEqual(200);
      res.json().then((json) => {
        expect(json.department.info).toEqual({
